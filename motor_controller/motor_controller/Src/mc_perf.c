...
void MC_Perf_Measure_Init(MC_Perf_Handle_t *pHandle) {
  ...
  if (MC_NULL == pHandle) {
    ...
    if (DWT->CTRL != 0U) { /* Check if DWT is present. */
      DWT->CYCCNT = 0;
...
/**
 * @brief  It returns the current CPU load of both High and Medium frequency
 * tasks.
 * @param  pHandle: handler of the performance measurement component.
 * @retval CPU load.
 */
float_t MC_Perf_GetCPU_Load(const MC_Perf_Handle_t *pHandle) {
  ...
  if (MC_NULL == pHandle) {
    ...
    MFT_cpu_loadM1 =
        (((float_t)pHandle->MC_Perf_TraceLog[MEASURE_TSK_MediumFrequencyTaskM1]
              .DeltaTimeInCycle /
          (float_t)SYSCLK_FREQ) *
         (float_t)MEDIUM_FREQUENCY_TASK_RATE);
    HFT_cpu_loadM1 =
        (((float_t)pHandle->MC_Perf_TraceLog[MEASURE_TSK_HighFrequencyTaskM1]
              .DeltaTimeInCycle /
          (float_t)SYSCLK_FREQ) *
...
/**
 * @brief  It returns the maximum CPU load of both High and Medium frequency
 * tasks.
 * @param  pHandle: handler of the performance measurement component.
...
/**
 * @brief  It returns the minimum CPU load of both High and Medium frequency
 * tasks.
 * @param  pHandle: handler of the performance measurement component.
...
#endif
    if (pHandle->MC_Perf_TraceLog[MEASURE_TSK_MediumFrequencyTaskM1].min !=
        UINT32_MAX) {
      cpu_load_acc +=
          (((float_t)pHandle
                ->MC_Perf_TraceLog[MEASURE_TSK_MediumFrequencyTaskM1]
                .min /
            (float_t)SYSCLK_FREQ) *
           (float_t)MEDIUM_FREQUENCY_TASK_RATE);
    }
    if (pHandle->MC_Perf_TraceLog[MEASURE_TSK_HighFrequencyTaskM1].min !=
        UINT32_MAX) {
      cpu_load_acc +=
          (((float_t)pHandle->MC_Perf_TraceLog[MEASURE_TSK_HighFrequencyTaskM1]
                .min /
