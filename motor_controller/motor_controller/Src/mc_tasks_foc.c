...
#define M2_CHARGE_BOOT_CAP_TICKS                                               \
  (((uint16_t)SYS_TICK_FREQUENCY * (uint16_t)10) / 1000U)
#define M2_CHARGE_BOOT_CAP_DUTY_CYCLES                                         \
  ((uint32_t)0 * ((uint32_t)PWM_PERIOD_CYCLES2 / 2U))
...
__weak void FOC_Init(void) {
  ...
  /**************************************/
  RUC_Init(&RevUpControlM1, pSTC[M1], &VirtualSpeedSensorM1, &STO_M1,
           pwmcHandle[M1]);
 ...
 * present state of its state machine. In particular, duties requiring a
 * periodic execution at a medium frequency rate (such as the speed controller
 * for instance) are executed here.
 */
    ...
    if (MCI_GetOccurredFaults(&Mci[M1]) == MC_NO_FAULTS) {
      ...
      case START: {
        if (MCI_STOP == Mci[M1].DirectCommand) {
          ...
          /* Mechanical speed as imposed by the Virtual Speed Sensor during the
           * Rev Up phase. */
          ...
          if (!RUC_Exec(&RevUpControlM1)) {
             ...
             * Compute the Iq reference current as configured in the Rev Up
             * sequence */
            IqdRef.q = STC_CalcTorqueReference(pSTC[M1]);
            IqdRef.d = FOCVars[M1].UserIdref;
            /* Iqd reference current used by the High Frequency Loop to generate
             * the PWM output */
            FOCVars[M1].Iqdref = IqdRef;
          }

          (void)VSS_CalcAvrgMecSpeedUnit(&VirtualSpeedSensorM1,
                                         &hForcedMecSpeedUnit);

          /* Check that startup stage where the observer has to be used has been
           * reached */
          if (true == RUC_FirstAccelerationStageReached(&RevUpControlM1)) {
            ObserverConverged =
                STO_PLL_IsObserverConverged(&STO_PLL_M1, &hForcedMecSpeedUnit);
            STO_SetDirection(&STO_PLL_M1,
                             (int8_t)MCI_GetImposedMotorDirection(&Mci[M1]));

            (void)VSS_SetStartTransition(&VirtualSpeedSensorM1,
                                         ObserverConverged);
          } else {
            ObserverConverged = false;
          }
          if (ObserverConverged) {
            qd_t StatorCurrent = MCM_Park(FOCVars[M1].Ialphabeta,
                                          SPD_GetElAngle(&STO_PLL_M1._Super));

            /* Start switch over ramp. This ramp will transition from the revup
             * to the closed loop FOC */
            REMNG_Init(pREMNG[M1]);
            (void)REMNG_ExecRamp(pREMNG[M1], FOCVars[M1].Iqdref.q, 0);
            (void)REMNG_ExecRamp(pREMNG[M1], StatorCurrent.q,
                                 TRANSITION_DURATION);
        ...
        if (MCI_STOP == Mci[M1].DirectCommand) {
          ...
          if (!RUC_Exec(&RevUpControlM1)) {
            ...
            /* Compute the virtual speed and positions of the rotor.
               The function returns true if the virtual speed is in the
               reliability range */
            LoopClosed = VSS_CalcAvrgMecSpeedUnit(&VirtualSpeedSensorM1,
                                                  &hForcedMecSpeedUnit);
            ...
            /* If any of the above conditions is true, the loop is considered
               closed. The state machine transitions to the RUN state */
            if (true == LoopClosed) {
              ...
              PID_SetIntegralTerm(
                  &PIDSpeedHandle_M1,
                  (((int32_t)FOCVars[M1].Iqdref.q *
                    (int16_t)PID_GetKIDivisor(&PIDSpeedHandle_M1)) /
              ...
              STC_SetSpeedSensor(
                  pSTC[M1], &STO_PLL_M1._Super); /* Observer has converged */
              FOC_InitAdditionalMethods(M1);
              FOC_CalcCurrRef(M1);
              STC_ForceSpeedReferenceToCurrentSpeed(
                  pSTC[M1]); /* Init the reference speed to current speed */
              MCI_ExecBufferedCommands(
                  &Mci[M1]); /* Exec the speed ramp after changing of the speed
                                sensor */
              Mci[M1].State = RUN;
            }
          }
      ...
      case STOP: {
        if (TSK_StopPermanencyTimeHasElapsedM1()) {
          ...
          STC_SetSpeedSensor(pSTC[M1],
                             &VirtualSpeedSensorM1._Super); /* Sensor-less */
 ...
 */
__weak void
FOC_InitAdditionalMethods(uint8_t bMotor) // cstat !RED-func-no-effect
{
  if (M_NONE == bMotor) {
    /* Nothing to do */
  } else {
...
/**
 * @brief  Executes the Motor Control duties that require a high frequency rate
 * and a precise timing.
 *
 *  This is mainly the FOC current control loop. It is executed depending on the
 * state of the Motor Control subsystem (see the state machine(s)).
 *
...
__weak uint8_t FOC_HighFrequencyTask(uint8_t bMotorNbr) {
  ...
  if (hFOCreturn == MC_DURATION) {
    ...
    bool IsAccelerationStageReached =
        RUC_FirstAccelerationStageReached(&RevUpControlM1);
    STO_Inputs.Ialfa_beta = FOCVars[M1].Ialphabeta; /* Only if sensorless */
    STO_Inputs.Vbus = VBS_GetAvBusVoltage_d(
        &(BusVoltageSensor_M1._Super)); /* Only for sensorless */
