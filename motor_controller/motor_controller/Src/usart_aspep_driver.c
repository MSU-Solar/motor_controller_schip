...
/**
 * @brief  Initialization of the Hardware used for data transmission and
 * reception.
 *
 * @param  pHandle Handler of the current instance of the UASPEP component
 */
void UASPEP_INIT(void *pHWHandle) {
  UASPEP_Handle_t *pHandle =
      (UASPEP_Handle_t *)pHWHandle; // cstat !MISRAC2012-Rule-11.5
...
void UASPEP_TX_INIT(UASPEP_Handle_t *pHandle) {
  ...
  if (NULL == pHandle) {
    ...
    /* Write the USART_TDR register address in the DMA control register to
     * configure it as the destination of the transfer */
    // cstat !MISRAC2012-Rule-11.4
    LL_DMA_SetPeriphAddress(pHandle->txDMA, pHandle->txChannel,
                            (uint32_t)&pHandle->USARTx->TDR);

    /* Clear UART ISR */
    LL_USART_ClearFlag_TC(pHandle->USARTx);

    /* DMA end of transfer on UART TX channel completion is not activated */
    /* We prefer to activate UART TC itself to avoid to trig IT while queued
     * data are still to be transmitted */
#ifdef NULL_PTR_CHECK_USA_ASP_DRV
  }
#endif
}
...
void UASPEP_RX_INIT(UASPEP_Handle_t *pHandle) {
  ...
  if (NULL == pHandle) {
    ...
    /* Write the USART_RDR register address in the DMA control register to
     * configure it as the source of the transfer */
    // cstat !MISRAC2012-Rule-11.4
    LL_DMA_SetPeriphAddress(pHandle->rxDMA, pHandle->rxChannel,
                            (uint32_t)&pHandle->USARTx->RDR);
