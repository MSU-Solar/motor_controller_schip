 ...
 * @brief   This file provides firmware functions that implement the MCP
 *protocol
 *
 *
 ******************************************************************************
 ...
 *
 * These components implement the features needed to drive and monitor motor
 * control applications embedded in STM32 MCUs. They mainly focus on the
 * communication with the controller, both on the receiving and the transmitting
 * end.
 *
 * @{
 */
...
/**
 * @brief  Parses the payload in the received packet and call the required
 * function in order to modify a value.
 *
 * The function called depends on the targeted motor and/or targeted register :
 * RI_SetRegisterGlobal or RI_SetRegisterMotorX.
 *
 ...
 *
 * @retval Returns #MCP_CMD_OK if the command is acknowledged and #MCP_CMD_NOK
 * if not.
 */
uint8_t RI_SetRegCommandParser(MCP_Handle_t *pHandle,
                               uint16_t txSyncFreeSpace) {
  ...
  if (MC_NULL == pHandle) {
    ...
    uint8_t typeID;
    uint8_t motorID;
    uint8_t (*SetRegFcts[NBR_OF_MOTORS + 1])(uint16_t, uint8_t, uint8_t *,
                                             uint16_t *, int16_t) = {
        &RI_SetRegisterGlobal, &RI_SetRegisterMotor1};
    ...
    while (rxLength > 0) {
      ...
      if (motorID > NBR_OF_MOTORS) {
        ...
        accessResult =
            SetRegFcts[motorID](regID, typeID, rxData, &size, rxLength);
        /* Prepare next data*/
        ...
        if ((1U == number_of_item) && (0 == rxLength)) {
          retVal = accessResult;
        } else { /* Store the result for each access to be able to report
                    failing access */
          if (txSyncFreeSpace != 0) {
            *txData = accessResult;
            ...
            if ((accessResult == MCP_ERROR_BAD_DATA_TYPE) ||
                (accessResult ==
                 MCP_ERROR_BAD_RAW_FORMAT)) { /* From this point we are not able
                                                 to continue to decode CMD
                                                 buffer*/
              /* We stop the parsing */
              rxLength = 0;
            }
            ...
            /* If we reach this state, chances are high the command was badly
             * formated or received */
            rxLength = 0;
...
/**
 * @brief  Parses the payload in the received packet and call the required
 * function in order to return a value.
 *
 * The function called depends on the targeted motor and/or targeted register :
 * RI_GetRegisterGlobal or RI_GetRegisterMotorX.
 *
 * @param  pHandle Handler of the current instance of the MCP component
 * @param  txSyncFreeSpace Space available for synchronous transmission
 *
...
uint8_t RI_GetRegCommandParser(MCP_Handle_t *pHandle,
  ...
  if (MC_NULL == pHandle) {
    ...
    uint8_t typeID;
    uint8_t motorID;
    uint8_t (*GetRegFcts[NBR_OF_MOTORS + 1])(uint16_t, uint8_t, uint8_t *,
                                             uint16_t *, int16_t) = {
        &RI_GetRegisterGlobal, &RI_GetRegisterMotor1};
    pHandle->txLength = 0;
    while (rxLength > 0U) {
      dataElementID = (uint16_t *)rxData;
      ...
      if (motorID > NBR_OF_MOTORS) {
        ...
        retVal =
            GetRegFcts[motorID](regID, typeID, txData, &size, freeSpaceS16);
...
/**
 * @brief  Parses the header from the received packet and call the required
 * function depending on the command sent by the controller device.
 *
 * @param  pHandle Handler of the current instance of the MCP component
 */
void MCP_ReceivedPacket(MCP_Handle_t *pHandle) {
  ...
  } else /* Length is 0, this is a request to send back the last packet */
  {
    ...
    /* Commands requiering payload response must be aware of space available for
     * the payload */
    /* Last byte is reserved for MCP response*/
    txSyncFreeSpace = (int16_t)pHandle->pTransportLayer->txSyncMaxPayload - 1;

    /* Initialization of the tx length, command which send back data has to
     * increment the txLength (case of Read register) */
    pHandle->txLength = 0U;
    ...
    case PROFILER_CMD: {
      MCPResponse = MC_ProfilerCommand(pHandle->rxLength, pHandle->rxBuffer,
                                       txSyncFreeSpace, &pHandle->txLength,
                                       pHandle->txBuffer);
      break;
    }

    case MCP_USER_CMD: {
      if ((userCommand < MCP_USER_CALLBACK_MAX) &&
          (MCP_UserCallBack[userCommand] != NULL)) {
        MCPResponse = MCP_UserCallBack[userCommand](
            pHandle->rxLength, pHandle->rxBuffer, txSyncFreeSpace,
