 ...
 * @brief   This file provides firmware functions that implement the following
 ...
 *
 * - The generation of the Space Vector Pulse Width Modulation on the motor's
 * phases
 * - The sampling of the actual motor's phases current
 *
 * Both these features are closely related as the instants when the values of
 * the phase currents should be sampled by the ADC channels are basically
 * triggered by the timers used to generate the duty cycles for the PWM.
 *
 * Several implementation of PWM and Current Feedback components are provided by
 * the Motor Control SDK to account for the specificities of the application:
 *
 * - The selected MCU: the number of ADCs available on a given MCU, the presence
 * of internal comparators or OpAmps, for instance, lead to different
 * implementation of this feature
 * - The Current sensing topology also has an impact on the firmware:
 * implementations are provided for Insulated Current Sensors, Single Shunt and
 * Three Shunt resistors current sensing topologies
 *
 * The choice of the implementation mostly depend on these two factors and is
 * performed by the Motor Control Workbench tool.
 *
 * All these implementations are built on a base PWM & Current Feedback
 * component that they extend and that provides the functions and data that are
 * common to all of them. This base component is never used directly as it does
 * not provide a complete implementation of the features. Rather, its handle
 * structure (PWMC_Handle) is reused by all the PWM & Current Feedback specific
 * implementations and the functions it provides form the API of the PWM and
 * Current feedback feature. Calling them results in calling functions of the
 * component that actually implement the feature. See PWMC_Handle for more
...
/**
 * @brief  Sets the calibrated @p offsets for each of the phases in the @p
 * pHandle handler. In case of single shunt only phase A is relevant.
 *
 */
// cstat !MISRAC2012-Rule-8.13 !RED-func-no-effect
__weak void PWMC_SetOffsetCalib(PWMC_Handle_t *pHandle,
...
/**
 * @brief  Gets the calibrated @p offsets for each of the phases in the @p
 * pHandle handler. In case of single shunt only phase A is relevant.
 *
 */
// cstat !MISRAC2012-Rule-8.13 !RED-func-no-effect
__weak void PWMC_GetOffsetCalib(PWMC_Handle_t *pHandle,
                                PolarizationOffsets_t *offsets) {
  if (MC_NULL == pHandle) {
    /* Nothing to do */
  } else {
...
/**
 * @brief  Converts input voltages @f$ V_{\alpha} @f$ and @f$ V_{\beta} @f$ into
 * PWM duty cycles and feed them to the inverter.
 *
 * This function computes the time during which the transistors of each phase
 * are to be switched on in a PWM cycle in order to achieve the reference phase
 * voltage set by @p Valfa_beta. The function then programs the resulting duty
 * cycles in the related timer channels. It also sets the phase current sampling
 * point for the next PWM cycle accordingly.
 *
 * This function is used in the FOC frequency loop and needs to complete itself
 * before the next PWM cycle starts in order for the duty cycles it computes to
 * be taken into account. Failing to do so (for instance because the PWM
 * Frequency is too high) results in the function returning #MC_DURATION which
 * entails a Motor Control Fault that stops the motor.
 *
 * @param  pHandle: Handler of the current instance of the PWM component.
 * @param  Valfa_beta: Voltage Components expressed in the @f$(\alpha, \beta)@f$
 * reference frame.
 * @retval #MC_NO_ERROR if no error occurred or #MC_DURATION if the duty cycles
 * were set too late for being taken into account in the next PWM cycle.
 */
 ...
 * @retval true if the current calibration has been completed, **false** if it
 * is still ongoing.
 */
 ...
 *
 * This function is called for setting low pass filter on Iq and Id, before
 * getting transformed in Ia and Ib by the Reverse Park function.
 *
 ...
 */
static inline int32_t PWMC_LowPassFilter(int32_t in, int32_t *out_buf,
                                         int32_t t) {
  int32_t x;
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == out_buf) {
    ...
    *out_buf = (*out_buf) + ((in - ((*out_buf) >> 15)) * t);
    x = (*out_buf) >> 15; // cstat !MISRAC2012-Rule-1.3_n !ATH-shift-neg
 ...
 */
void PWMC_CalcPhaseCurrentsEst(PWMC_Handle_t *pHandle, qd_t Iqd,
                               int16_t hElAngledpp) {
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == pHandle) {
    /* Nothing to do */
 ...
 *
 * @param  pHandle:  Handler of the current instance of the PWM component.
 * @param  hDACVref: Value of DAC reference voltage to be applied expressed as a
 *16bit unsigned integer. Min value: 0 (0 V) Max value: 65536 (VDD_DAC)
 */
// cstat !MISRAC2012-Rule-8.13 !RED-func-no-effect
...
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if ((MC_NULL == pHandle) ||
      (MC_NULL == pHandle->pFctOCPSetReferenceVoltage)) {
    /* Nothing to do */
  } else {
#endif
...
/** @brief  Returns the status of the Discontinuous PWM Mode stored in the @p
 * pHandle PWMC component.
 *
...
/** @brief  Enables the RL detection mode by calling the function in @p pHandle
 * PWMC component.
 *
 */
// cstat !MISRAC2012-Rule-8.13 !RED-func-no-effect
...
/** @brief  Disables the RL detection mode by calling the function in @p pHandle
 * PWMC component.
 *
 */
// cstat !MISRAC2012-Rule-8.13 !RED-func-no-effect
...
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if ((MC_NULL == pHandle) ||
      (MC_NULL == pHandle->pFctRLDetectionModeDisable)) {
    /* Nothing to do */
  } else {
#endif
 ...
 * @retval #MC_NO_ERROR if the Duty Cycle could be applied on time for the next
 * PWM period. Returns #MC_DURATION otherwise.
 */
    ...
    PWMC_Handle_t *pHandle, uint16_t hDuty) // cstat !MISRAC2012-Rule-8.13
{
  ...
  if ((MC_NULL == pHandle) ||
      (MC_NULL == pHandle->pFctRLDetectionModeSetDuty)) {
    /* Nothing to do */
  } else {
...
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if ((MC_NULL == pHandle) ||
      (MC_NULL == pHandle->pFctRLTurnOnLowSidesAndStart)) {
    /* Nothing to do */
  } else {
#endif
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to get phases
 * current.
 *
 ...
 */
__weak void
PWMC_RegisterGetPhaseCurrentsCallBack(PWMC_GetPhaseCurr_Cb_t pCallBack,
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to execute a
 * calibration of the current sensing system.
 *
 * @param pCallBack: Pointer on the generic callback.
 * @param pHandle: Handler of the current instance of the PWMC component.
 */
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to turn low
 * sides on.
 *
 ...
 */
__weak void
PWMC_RegisterTurnOnLowSidesCallBack(PWMC_TurnOnLowSides_Cb_t pCallBack,
                                    PWMC_Handle_t *pHandle) {
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == pHandle) {
    /* Nothing to do */
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to compute ADC
 * sampling point.
 *
 * @param pCallBack: Pointer on the callback which sets the sampling point
 * depending on the sector.
 * @param pHandle: Handler of the current instance of the PWMC component.
 */
__weak void
PWMC_RegisterSampPointSectXCallBack(PWMC_SetSampPointSectX_Cb_t pCallBack,
                                    PWMC_Handle_t *pHandle) {
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == pHandle) {
    /* Nothing to do */
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to set the
 * reference voltage for the overcurrent protection.
 *
 * @param pCallBack: Pointer on the callback which sets the reference voltage.
 * @param pHandle: Handler of the current instance of the PWMC component.
 */
__weak void
PWMC_RegisterOCPSetRefVoltageCallBack(PWMC_SetOcpRefVolt_Cb_t pCallBack,
                                      PWMC_Handle_t *pHandle) {
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == pHandle) {
    /* Nothing to do */
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to enable the
 * R/L detection mode.
 *
 * @param pCallBack: Pointer on the generic callback.
 * @param pHandle: Handler of the current instance of the PWMC component.
 */
__weak void
PWMC_RegisterRLDetectionModeEnableCallBack(PWMC_Generic_Cb_t pCallBack,
                                           PWMC_Handle_t *pHandle) {
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == pHandle) {
    /* Nothing to do */
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to disable the
 * R/L detection mode.
 *
 * @param pCallBack: Pointer on the generic callback.
 * @param pHandle: Handler of the current instance of the PWMC component.
 */
__weak void
PWMC_RegisterRLDetectionModeDisableCallBack(PWMC_Generic_Cb_t pCallBack,
                                            PWMC_Handle_t *pHandle) {
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == pHandle) {
    /* Nothing to do */
...
/**
 * @brief Sets the Callback that the PWMC component shall invoke to set the duty
 * cycle for the R/L detection mode.
 *
 * @param pCallBack: Pointer on the callback which sets the duty cycle.
 * @param pHandle: Handler of the current instance of the PWMC component.
 */
__weak void
PWMC_RegisterRLDetectionModeSetDutyCallBack(PWMC_RLDetectSetDuty_Cb_t pCallBack,
                                            PWMC_Handle_t *pHandle) {
#ifdef NULL_PTR_CHECK_PWR_CUR_FDB
  if (MC_NULL == pHandle) {
    /* Nothing to do */
