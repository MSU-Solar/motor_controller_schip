 ...
 *
 *  This interface allows for performing basic operations on the motor driven by
 * a Motor Control SDK based application. With it, motors can be started and
 * stopped, speed or torque ramps can be programmed and executed and information
 * on the state of the motor can be retrieved, among others.
 *
 *  These functions aims at being the main interface used by an application to
 * control the motor.
 *
 * @{
 */
 ...
 *         value.
 *
 *  This command is executed immediately if the target motor's state machine is
 * in the #RUN state. Otherwise, it is buffered and its execution is delayed
 * until This state is reached.
 *
 * Users can check the status of the command by calling the
 * MCI_IsCommandAcknowledged() function.
 *
 ...
 *         the ramp. This value represents actually the $I_q$ current expressed
 * in digit. To convert current expressed in Amps to current expressed in digit
 ...
 *         the ramp. This value represents actually the $I_q$ current expressed
 * in Ampere. Here the formula for conversion from current in Ampere to digit:
 ...
 */
__weak void MCI_ExecTorqueRamp_F(MCI_Handle_t *pHandle,
                                 const float_t FinalTorque,
                                 uint16_t hDurationms) {
#ifdef NULL_PTR_CHECK_MC_INT
  if (MC_NULL == pHandle) {
    /* Nothing to do */
...
/**
  ...
  *  This command is executed immediately if the target motor's state machine is
  in
  * the #RUN state. Otherwise, it is buffered and its execution is delayed until
  This
  * state is reached.
  *
  * Users can check the status of the command by calling the
...
/**
  ...
  *
  * @param  pHandle Pointer on the component instance to work on.
  * @param  IqdRef current (A) references on qd reference frame in qd_f_t
  format.
  *
  *  This command is executed immediately if the target motor's state machine is
  in
 ...
 *         procedure. If the state machine is in IDLE state the command is
 * executed instantaneously otherwise the command is discarded. User must take
 *         care of this possibility by checking the return value.\n
 *         <B>Note:</B> The MCI_StartOffsetMeasurments command is used to begin
 * phase offset calibration procedure moving the state machine from IDLE state
 * to OFFSET_CALIB. The command MCI_StartOffsetMeasurments is not blocking the
 * execution of project until the measurments are done; to do this, the user
 ...
 *
 * The offset values are written in the PolarizationOffsets structure provided
 * that they have been previously provided for the Motor Control subsystem or
 * measured by it.
 *
 * If the offset have not previously been provided to the Motor Control
 * subsystem or if it has not measured them the function returns false and
 * nothing is written in the PolarizationOffsets structure.
 *
 * @param  pHandle Pointer on the component instance to work on.
 * @param  PolarizationOffsets Pointer on ploarization offset structure in which
 * offsets will be written
 * @retval returns true if the command is successfully executed; returns false
 * otherwise.
 */
__weak bool
MCI_GetCalibratedOffsetsMotor(MCI_Handle_t *pHandle,
 ...
 *
 * @param  pHandle Pointer on the component instance to work on.
 * @param  PolarizationOffsets Pointer on ploarization offset structure that
 * contains phase A, and C values.
 * @retval Returns true if the command is successfully executed
 *         otherwise it return false.
 */
__weak bool
MCI_SetCalibratedOffsetsMotor(MCI_Handle_t *pHandle,
                              PolarizationOffsets_t *PolarizationOffsets) {
  bool retVal = false;
#ifdef NULL_PTR_CHECK_MC_INT
  if (MC_NULL == pHandle) {
 ...
 *
 *  This function must be called before the motor can be started again when a
 * fault condition has occured. It clears the faults status and resets the state
 * machine of the target motor to the #IDLE state provided that there is no
 * active fault condition anymore.
 *
 *  If the state machine of the target motor is in the #FAULT_OVER state, the
 * function clears the list of past faults, transitions to the #IDLE state and
 * returns true. Otherwise, it oes nothing and returns false.
 *
...
#endif
    /* Set current errors */
    pHandle->CurrentFaults =
        (pHandle->CurrentFaults | hSetErrors) & (~hResetErrors);
 ...
 */
__weak MCI_State_t
MCI_GetSTMState(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
...
/**
 * @brief Returns the list of non-acknowledged faults that occured on the target
 * motor
 *
 * This function returns a bitfield indicating the faults that occured since the
 * state machine of the target motor has been moved into the #FAULT_NOW state.
 *
 * Possible error codes are listed in the @ref fault_codes "Fault codes"
 * section.
 *
 * @param  pHandle Pointer on the target motor drive structure.
...
/**
 * @brief Returns the list of faults that are currently active on the target
 * motor
 *
 * This function returns a bitfield that indicates faults that occured on the
 * Motor Control subsystem for the target motor and that are still active (the
 * conditions that triggered the faults returned are still true).
 *
 * Possible error codes are listed in the @ref fault_codes "Fault codes"
 * section.
 *
 ...
 */
__weak uint16_t
MCI_GetCurrentFaults(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
  return ((MC_NULL == pHandle) ? MC_SW_ERROR
                               : (uint16_t)pHandle->CurrentFaults);
#else
...
/**
 * @brief Returns the lists of current and past faults that occurred on the
 * target motor
 *
 *  This function returns two bitfields containing information about the faults
 * currently present and the faults occurred since the state machine has been
 * moved into the #FAULT_NOW state.
 *
 * These two bitfields are 16 bits wide each and are concatenated into the
 * 32-bit data. The 16 most significant bits contains the status of the current
 * faults while that of the past faults is in the 16 least significant bits.
 *
 ...
 */
__weak uint32_t
MCI_GetFaultState(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
 ...
 */
__weak int16_t MCI_GetImposedMotorDirection(
    MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
  int16_t retVal = 1;
 ...
 */
__weak float_t MCI_GetLastRampFinalTorque_F(
    MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
  float_t retVal = 0;
  ...
  if (MC_NULL == pHandle) {
    ...
    retVal =
        ((float_t)pHandle->hFinalTorque * (float_t)pHandle->pScale->current);
  }
  return (retVal);
#else
 ...
 */
__weak uint16_t MCI_GetLastRampFinalDuration(
    MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
  uint16_t retVal = 0;
 ...
 */
__weak float_t MCI_GetLastRampFinalSpeed_F(
    MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
  float_t reVal = 0.0f;
  ...
  if (MC_NULL == pHandle) {
    ...
    reVal = (((float_t)pHandle->hFinalSpeed * (float_t)U_RPM) /
             (float_t)SPEED_UNIT);
  }
  return (reVal);
}
 ...
 * will be removed in a future version of the MCSDK. Use MCI_StopRamp() instead.
 */
...
/**
 * @brief  Returns the current mechanical rotor speed reference expressed in the
...
/**
 * @brief  Returns the current mechanical rotor speed reference expressed in
 * rpm.
 *
 * @param  pHandle Pointer on the component instance to work on.
 *
...
#endif
    iab.a =
        (float_t)((float_t)pHandle->pFOCVars->Iab.a * pHandle->pScale->current);
    iab.b =
        (float_t)((float_t)pHandle->pFOCVars->Iab.b * pHandle->pScale->current);
#ifdef NULL_PTR_CHECK_MC_INT
  }
#endif
  return (iab);
 ...
 */
__weak alphabeta_t
MCI_GetIalphabeta(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
  alphabeta_t tempVal;
...
#endif
    iqd.d =
        (float_t)((float_t)pHandle->pFOCVars->Iqd.d * pHandle->pScale->current);
    iqd.q =
        (float_t)((float_t)pHandle->pFOCVars->Iqd.q * pHandle->pScale->current);
#ifdef NULL_PTR_CHECK_MC_INT
  }
#endif
  return (iqd);
 ...
 */
__weak qd_f_t
MCI_GetIqdref_F(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
  qd_f_t iqdref;
#ifdef NULL_PTR_CHECK_MC_INT
  if (MC_NULL == pHandle) {
...
#endif
    iqdref.d = (float_t)((float_t)pHandle->pFOCVars->Iqdref.d *
                         pHandle->pScale->current);
    iqdref.q = (float_t)((float_t)pHandle->pFOCVars->Iqdref.q *
                         pHandle->pScale->current);
#ifdef NULL_PTR_CHECK_MC_INT
  }
#endif
 ...
 */
__weak alphabeta_t
MCI_GetValphabeta(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
  alphabeta_t tempVal;
 ...
 */
__weak int16_t
MCI_GetElAngledpp(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
 ...
 */
__weak int16_t
MCI_GetTeref(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
 ...
 */
__weak float_t
MCI_GetTeref_F(MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_MC_INT
...
#else
  return ((float_t)pHandle->pFOCVars->hTeref *
          (float_t)pHandle->pScale->current);
#endif
}
 ...
 */
__weak int16_t MCI_GetPhaseCurrentAmplitude(
    MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
 ...
 */
__weak int16_t MCI_GetPhaseVoltageAmplitude(
    MCI_Handle_t *pHandle) // cstat !MISRAC2012-Rule-8.13
{
  int16_t temp_wAux;
#ifdef NULL_PTR_CHECK_MC_INT
