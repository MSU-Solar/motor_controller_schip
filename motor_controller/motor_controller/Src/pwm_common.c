 ...
 *
 * Uses TIM2 as a temporary timer to achieve synchronization between PWM
 * signals. When this function is called, TIM1 and/or TIM8 must be in a frozen
 * state with CNT, ARR, REP RATE and trigger correctly set (these settings are
 ...
 *
 * If the polarization exceeds the number of needed PWM cycles, it reports an
 * error.
 *
...
__weak void waitForPolarizationEnd(TIM_TypeDef *TIMx, uint16_t *SWerror,
                                   uint8_t repCnt, volatile uint8_t *cnt) {
