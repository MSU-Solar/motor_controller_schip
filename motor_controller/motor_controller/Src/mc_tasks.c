 ...
 *
 * This function is to be called periodically at least at the Medium Frequency
 * task rate (It is typically called on the Systick interrupt). Exact invokation
 * rate is the Speed regulator execution rate set in the Motor Contorl
 * Workbench.
 *
  ...
  if (0U == bMCBootCompleted) {
    ...
    if (hMFTaskCounterM1 > 0u) {
      ...
      MCP_Over_UartA.rxBuffer =
          MCP_Over_UartA.pTransportLayer->fRXPacketProcess(
              MCP_Over_UartA.pTransportLayer, &MCP_Over_UartA.rxLength);
      if (0U == MCP_Over_UartA.rxBuffer) {
        /* Nothing to do */
      } else {
        /* Synchronous answer */
        if (0U == MCP_Over_UartA.pTransportLayer->fGetBuffer(
                      MCP_Over_UartA.pTransportLayer,
                      (void **)&MCP_Over_UartA
                          .txBuffer, // cstat !MISRAC2012-Rule-11.3
                      MCTL_SYNC)) {
          /* No buffer available to build the answer ... should not occur */
        } else {
          MCP_ReceivedPacket(&MCP_Over_UartA);
          MCP_Over_UartA.pTransportLayer->fSendPacket(
              MCP_Over_UartA.pTransportLayer, MCP_Over_UartA.txBuffer,
...
/**
 * @brief  Executes safety checks (e.g. bus voltage and temperature) for all
 * drive instances.
 *
...
__weak void TSK_SafetyTask_PWMOFF(uint8_t bMotor) {
  ...
  /* Due to warning array subscript 1 is above array bounds of PWMC_Handle_t
   * *[1] [-Warray-bounds] */
  CodeReturn |= PWMC_IsFaultOccurred(
      pwmcHandle[lbmotor]); /* check for fault. It return MC_OVER_CURR or
                 MC_NO_FAULTS (for STM32F30x can return MC_OVER_VOLT in case of
                 HW Overvoltage) */

  if (M1 == bMotor) {
    uint16_t rawValueM1 = RCM_ExecRegularConv(&VbusRegConv_M1);
    CodeReturn |=
        errMask[bMotor] & RVBS_CalcAvVbus(&BusVoltageSensor_M1, rawValueM1);
  } else {
    /* Nothing to do */
  }
  MCI_FaultProcessing(&Mci[bMotor], CodeReturn,
                      ~CodeReturn); /* Process faults */
...
/**
 * @brief  Puts the Motor Control subsystem in in safety conditions on a Hard
 * Fault
 *
 *  This function is to be executed when a general hardware failure has been
 * detected by the microcontroller and is used to put the system in safety
 * condition.
 */
...
/**
 * @brief  Locks GPIO pins used for Motor Control to prevent accidental
 * reconfiguration.
 */
