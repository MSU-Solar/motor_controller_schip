...
const R3_1_Params_t R3_1_ParamsM1 = {
    ...
    .ADCx = ADC1,
    .ADCConfig =
        {(7U << ADC_JSQR_JSQ1_Pos) | (6U << ADC_JSQR_JSQ2_Pos) |
             1 << ADC_JSQR_JL_Pos |
             (LL_ADC_INJ_TRIG_EXT_TIM1_TRGO & ~ADC_INJ_TRIG_EXT_EDGE_DEFAULT),
         (1U << ADC_JSQR_JSQ1_Pos) | (6U << ADC_JSQR_JSQ2_Pos) |
             1 << ADC_JSQR_JL_Pos |
             (LL_ADC_INJ_TRIG_EXT_TIM1_TRGO & ~ADC_INJ_TRIG_EXT_EDGE_DEFAULT),
         (1U << ADC_JSQR_JSQ1_Pos) | (6U << ADC_JSQR_JSQ2_Pos) |
             1 << ADC_JSQR_JL_Pos |
             (LL_ADC_INJ_TRIG_EXT_TIM1_TRGO & ~ADC_INJ_TRIG_EXT_EDGE_DEFAULT),
         (1U << ADC_JSQR_JSQ1_Pos) | (7U << ADC_JSQR_JSQ2_Pos) |
             1 << ADC_JSQR_JL_Pos |
             (LL_ADC_INJ_TRIG_EXT_TIM1_TRGO & ~ADC_INJ_TRIG_EXT_EDGE_DEFAULT),
         (1U << ADC_JSQR_JSQ1_Pos) | (7U << ADC_JSQR_JSQ2_Pos) |
             1 << ADC_JSQR_JL_Pos |
    ...
    .Tcase3 =
        ((uint16_t)TDEAD + (uint16_t)TNOISE + (uint16_t)SAMPLING_TIME) / 2u,
    .TIMx = TIM1,
...
ScaleParams_t scaleParams_M1 = {
    .voltage =
        NOMINAL_BUS_VOLTAGE_V / (1.73205 * 32767), /* sqrt(3) = 1.73205 */
    .current = CURRENT_CONV_FACTOR_INV,
    .frequency =
        (1.15 * MAX_APPLICATION_SPEED_UNIT * U_RPM) / (32768 * SPEED_UNIT)};
